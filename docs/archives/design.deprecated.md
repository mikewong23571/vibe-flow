...
当前大多数现代 coding agent runtime，通常已经具备用某种 `agent_home` 或等价目录承载 agent instruction、tools、permissions、context policy 的能力。正因为这种能力配置已经由 agent runtime 自己管理，工具链才可以不介入 agent 能力建模，而只聚焦 task workflow、worker launch 和状态流转。
...
* task
* run
* agent_home reference
* worker launch
* state transition record
* error exit

mgr 负责根据 task_type 的编排 prompt、当前 task 状态和最近一次 run，选择下一个 worker 并推动任务流转。
...
* `agent_home` 是传给 agent runtime 的目录引用；对工具链而言它是不透明的。
...
* `run` 是 task 上由某个 worker 产生的一次执行记录。
...
`agent_home` 在工具链层只是一个目录。

这个目录的内部结构和语义，不属于工具链核心模型。

在背景层，可以认为现代 coding agent runtime 往往会通过这类目录或等价机制承载：
...
但这些能力如何组织、如何生效、如何实现隔离，都由 `agent_home` 对应的 runtime 或 launch 机制自己负责，不由工具链解释。

系统允许存在多个 agent_home。

mgr 和 worker 都可以在 launch 时拿到各自对应的 agent_home 目录。
...
```text
复用既有 agent runtime 的能力边界，让工具链不必自己建模 instruction、tools、permissions、context policy。
```
...
从运行视角看，一个 task 最终会体现为一串按 task_type 协议推进的 worker launch / run：

```text
task lifecycle = run1 -> run2 -> run3 -> ...
每个 run 对应一次 worker launch
直到 task done 或 error
```
...
```text
根据 task_type workflow 和 task 的最新 run，推动 collection 中的 task 进入下一个 worker、完成状态或 error 状态。
```
...
worker 的工作方式由其对应 runtime 如何消费 `agent_home`，以及 task_type / worker prompt 共同决定。

在工具链层，worker 相关的唯一关键动作是：
...
至于 launch 之后如何让 `agent_home` 生效、如何让 agent 具备对应能力、如何实现隔离，不属于工具链职责。
...
`run` 是 task 上一次 worker launch 的执行记录。
