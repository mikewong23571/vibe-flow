...
(defn codex-drive [target-root task mgr-run worker-launcher]
  (let [install-record (install/load-install! target-root)
        home-name (get-in install-record [:task-types (:task-type task) :mgr-home])
        home-dir (fs/path target-root ".workflow" "agent_homes" (name home-name))
