...
(defn prepare-run! [target-root task worker launcher]
  (let [prepare-spec (task-type/run-prepare-run-hook! target-root task worker launcher)
        run-id (util/uuid)
  ...
  (let [worker (:worker run)
        ...
        output-head (if commit?
                      (target-repo/commit-worktree! worktree
                                                    (str "preproject_stage2_toolchain_target_boundary " (:id task) " " (name worker)))
                      (:input-head run))]
    (assoc run
           :ended-at (util/now)
